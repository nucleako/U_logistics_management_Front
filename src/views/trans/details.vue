<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-waves class="filter-item" type="primary" icon="el-icon-search" style="margin-left: 10px;width:90px; margin-right:10px" @click="handleFilter">
        搜索
      </el-button>
      <el-button
        class="filter-item"
        style="margin-left: 10px;width:90px; margin-right:10px"
        icon="el-icon-refresh"
        @click="getRefresh"
      >
        刷新
      </el-button>
      <div v-for="item in list" :key="item.id" class="finishitem">
        <div class="top_item">
          <!-- <span>下单时间：{{item.OrderDate}}</span> -->
          <span style="margin-left: 50px;">运单编号：{{ item.id }}</span>
          <span style="margin-left:125px ;">运单总额：{{ item.money }}</span>
          <span style="margin-left: 125px;">订单编号：{{ item.OrderID }}</span>
          <span style="margin-left:125px ;">承运商编号：{{ item.CarrierId }}</span>
